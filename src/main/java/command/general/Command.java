...
public class Command implements CommandExecutor {
                    ...
                    new ArrayList<>(){{
                        ...
                        add(new Argument(
                                ...
                                (sender, args, values) -> {
                                    ...
                                    Bukkit.broadcastMessage("Succes! (1)");
                                },
                                "cool",
                                null
                        ));
                        add(new Argument(
                                ...
                                null,
                                (sender, args, value, index, values) -> new ArrayList<>() {{add("Coolsta!");}},
                                null,
                                null,
                                (sender, args, value, index, values) -> value,
                                (sender, args, values) -> {
                                    boolean global = (boolean) values.get("global");
                                    if (global) Bukkit.broadcastMessage("Global!");
                                    Bukkit.broadcastMessage("Succes! (2)");
                                },
                                "coolsta",
                                null
                        ));
                        add(new Argument(
                                (sender, args, value, index, values) -> value.equalsIgnoreCase("lol"),
                                null,
                                (sender, args, value, index, values) -> new ArrayList<>() {{add("lol");}},
                                null,
                                null,
                                (sender, args, value, index, values) -> value,
                                null,
                                ...
                                new ArrayList<>(){{
                                    add(new Argument(
                                            (sender, args, value, index, values) -> value.equalsIgnoreCase("cyp"),
                                            null,
                                            (sender, args, value, index, values) -> new ArrayList<>() {{add("cyp");}},
                                            null,
                                            (sender, args, value, index, values) -> Bukkit.broadcastMessage("where cyp?"),
                                            (sender, args, value, index, values) -> value,
                                            (sender, args, values) -> Bukkit.broadcastMessage("Cyp"),
                                            "cyp",
                                            null
                                    ));
                                }}
    ...
    public boolean onCommand(CommandSender sender, org.bukkit.command.Command command, String label, String[] args) {
        ...
        List<Argument> argumentsCopy = getArguments();
            ...
            if (currentOptionalArgument.isPresent()) {
                ...
                if (currentArgument.isValid == null || currentArgument.isValid.test(sender, finalArgs, arg, finalI, values)){
                    ...
                    argumentsCopy.stream().filter((a) -> a.modifier).forEach((ma) -> {
                        values.putIfAbsent(ma.key, false);
                    });
                    ...
                    if (i+1 == finalArgs.length) {
                        if (currentArgument.invoke != null) {
                            ...
                            List<Argument> inferiorArguments;
                            if (currentArgument.modifier) {
                                inferiorArguments = argumentsCopy;
                                inferiorArguments.remove(currentArgument);
                            } else {
                                inferiorArguments = currentArgument.followingArguments;
                            }
